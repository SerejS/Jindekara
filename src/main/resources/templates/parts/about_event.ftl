        ...
        <fieldset class="field_set">
            <form method="post" name="event" action="save">
                <ul>
                    ...
                    <li><input type="number" name="dateEnd" placeholder="Дата канца события"></li>
                    ...
                    <li><button type="submit" value="save" class="butBar">Добавить</button></li>
                </ul>
            </form>
        </fieldset>
    <#else>
        ${selected_event.getNameEvent()}<br>
        ${selected_event.getDateStart()}<br>
        ${selected_event.getDateEnd()}<br>
        ${selected_event.getYearStart()}<br>
        ${selected_event.getYearEnd()}<br>
        ${selected_event.getAffect().getName()}
    </#if>
</div>